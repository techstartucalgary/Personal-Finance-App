...
export async function getCategorySpending(params: {
  ...
  // Filter by date using transaction_date if available, otherwise created_at
  const filtered = (data ?? []).filter((row: any) => {
    const dateToUse = row.transaction_date
      ? row.transaction_date
      : row.created_at?.split("T")[0]; // Extract date part from timestamp
